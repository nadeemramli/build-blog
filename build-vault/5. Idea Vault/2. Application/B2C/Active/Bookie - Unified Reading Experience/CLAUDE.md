...
- **2026-07-13 — Catalog vs Library split; catalog as markdown files.** One md per book in `Catalog/` (frontmatter + latticework body), imported from the "Library of Books" sheet (~1,739 books). Status flow wishlist→queued→available→reading→done. Match files↔catalog by normalized title/author, rename to `Author - Title.ext` on confirm, record content hash. Acquisition = daily queue + Drop/ folder watcher; the z-library download click stays manual. See PVP-2 §Catalog & Acquisition.
