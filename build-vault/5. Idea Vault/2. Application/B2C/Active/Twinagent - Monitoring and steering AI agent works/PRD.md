...
Full notes: [Agent Notch Reference Notes](<Agent Notch Reference Notes.md>)
...
Plan usage across providers (Claude Pro/Max 5h + weekly windows, Codex 5h + weekly windows, others) via the `UsageSource` ladder in [Usage Tracking Research](<Usage Tracking Research.md>). Phase 1 covers the pure local-file sources: Claude Code JSONL usage records and Codex `token_count.rate_limits` payloads (which include `used_percent` and `resets_at` directly).
...
- **Usage tracking**: pluggable `UsageSource` per provider with a priority ladder — local files → reused OAuth → provider API → cookies/manual — each reading tagged exact/estimated/stale. See [Usage Tracking Research](<Usage Tracking Research.md>).
