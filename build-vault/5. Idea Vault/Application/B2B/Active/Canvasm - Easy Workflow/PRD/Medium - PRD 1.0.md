...
- **Practical Environment**: This is where users operationalize their data. They can connect data sources, transform data, visualize it and pipe it through operative nodes before "using" it in metric nodes. This environment enables users to define relationships between metrics, run simulations, and analyze their data-driven business models.
...
This dual-environment allows users to explore, brainstorm, build, navigate, and analyze their business architecture in a single, interactive workspace. The canvas is not a static drawing board; it is a live modeling tool where a company can map out and test its "fundamental formula"—the unique way of a business model translates inputs into value. By building directly in Canvasm, business architecture becomes more visible, tractable, and robust. Worth to note that **business architecture can be manifested in a form of User Journey, Funnel, Metric Trees, or Growth Model.**
 
- **Within Canvas:** 
	- **Node Creation:** Users can populate the canvas by creating **Cards**. These cards are the fundamental building blocks of the model, representing every component of the business system—from high-level `North Star` metrics to granular `Work/Action` items like experiments and initiatives. The process of adding nodes is the first step in translating abstract business concepts into a structured, logical model.
		- **Core:**
			- **Value Node:** Represents a core component in a business model's value chain, forming the backbone of the business's fundamental formula.
			- **Action Node:** Maps out specific tasks, representing either routine "Business-as-Usual" activities or new, hypothetical initiatives.
			- **Hypothesis Node:** A dedicated space for brainstorming and capturing new ideas within the business model.
		- **Artifacts:**
			- **Evidence Node:** A block-based text editor, similar to a digital notebook, where users can create reports that embed real-time data by referencing other nodes and their relationships.
			- **Metadata Node:** Enriches other nodes by providing additional context, descriptions, or properties.
		- **Data/Metric:**
			- **Metric Node:** A data-centric node that stores and acts as a proxy for data pulled from a Source Node. It serves as the primary element for tracking key business metrics.
			- **Source Node:** Manages data pipelines, allowing users to connect to data sources, perform basic transformations (like filtering), or run custom SQL/Python scripts for advanced data manipulation.
		- **Functional & Utility:**
			- **Visualization Node:** Pairs with Metric or Source Nodes to create standard charts and graphs, offering a quick way to visualize data.
			- **Operative Node:** A utility node for managing data flows. It allows users to apply custom formulas, use boolean operators, create conditional paths, add delays, or streamline date parameters for data sources. These can be managed individually or from a central control panel.
	- **Node Properties:** Every node on the canvas is a rich data object. By double-clicking a node, users can access its specific properties sheet on the right side of the screen, allowing them to configure and enrich each element of their model.
		- **Core:**
			- **Value Node:** 
				- **Title & Description**: Define the name and purpose of this core business component.
				- **Owner**: Assign a team or individual responsible for this value driver.
				- **Status**: Set a status like `On Track`, `At Risk`, or `Planned`.
			- **Action Node:**
				- **Title & Description**: Detail the specific task or initiative.
				- **Assignee & Due Date**: Assign responsibility and set a timeline for completion.
				- **Impact & Effort Score**: Quantify the expected impact versus the required effort.
			- **Hypothesis Node:**
				- **Title & Description**: Articulate the core idea or question to be tested.
				- **Confidence Score**: Rate the team's confidence in the hypothesis.
				- **Evidence Links**: Connect to Evidence Nodes that support or refute the idea.
		- **Artifacts:**
			- **Evidence Node:**
				- **Editor**: Access the full-featured, block-based text editor to write reports, embed other nodes, and analyze findings.
				- **Authors & Contributors**: Manage who can view and edit the document.
				- **Version History**: Track changes and revert to previous versions.
			- **Metadata Node:**
				- **Custom Fields**: Create and manage key-value pairs (e.g., `Region: EMEA`, `Priority: High`) to add structured data to other nodes.
				- **Target Node**: Select which node this metadata applies to.
		- **Data/Metric:**
			- **Metric Node:**
				- **Title & Description**: Define the metric's name and its business significance.
				- **Data Source**: Link to a Source Node or manual data entry.
				- **Goal & Thresholds**: Set target values and define thresholds for `good` and `bad` performance.
				- **Refresh Interval**: Configure how often the metric's data should be updated.
			- **Source Node**:
				- **Connection Type**: Select the data source (e.g., Google Sheets, Snowflake, PostgreSQL).
				- **Credentials & Query**: Securely input credentials and write the SQL/Python query to fetch data.
				- **Data Preview**: View a sample of the fetched data to ensure the query is correct.
		- **Functional & Utility:**
			- **Visualization Node:**
				- **Chart Type**: Select the visualization format (e.g., Line Chart, Bar Chart, Pie Chart).
				- **Data Mapping**: Assign data columns to the chart's axes (X and Y).
				- **Appearance**: Customize colors, labels, and titles for the chart.
			- **Operative Node:**
				- **Operation Type**: Choose the utility's function (e.g., Formula, Boolean (If/Else), Date Picker).
				- **Configuration**: Input the specific formula, define the conditional logic, or set the date range.
				- **Input/Output Mapping**: Define which nodes feed data into this utility and where the resulting data should be sent.
	- **Node Connectivity:** Users can draw directed edges (arrows) between nodes to create **Relationships**. This is a critical function, as these connections represent the logic of the business model. 
		- **Dynamic Edges:** Dynamic edge explicitly defines the relationship between two **Metric Cards / Metric Node** (e.g. `Causal`, `Deterministic`,`Probabilistic`, `Compositional`), turning the canvas from a collection of ideas into a coherent, causal map that can be analyzed and validated.
		- **Operative Edges**: Operative edge is simply to pass data between Nodes, and ensuring can use Operative Nodes in between to control Data Flow. (like Zapier). An operative edges will have a blurred out button (if far) next to it to run test/run execute.
		- **Simple Edges**: Simple edge is simply an edge to peg or connect something to a node. For an example, evidence to a node, this can't use Dynamic Edges and even Operative Edges. Same goes with Metric Card of Core/Value, Work/Action, Ideas/Hypothesis and Metadata. These type of nodes only uses simple edge
	- **Control Panel:** 
		- **Control Panel:** Built to manage operative nodes. Act as main control operator for each individual operative node.
		- **Flow Control:** Sees where there's error in your pipeline.
	- **Collaboration:**
		- **Share:** 
			- Sharing Canvas:
			- Sharing Dashboard: Canvasm have a unique way to develop Dashboard for user. Instead of manually picking up Cards to form a dashboard, Canvasm uses Grouping as the basis for formulating dashboard. Grouping Nodes together auto-create Dashboard. User able to select this as a part of their sharing
			- Sharing Evidence: 
		- **Comment:**
			- **Where:**
				- **Pin Comment:** On Canvas
				- I**nside Node Property**: In each of our node property/details, user can insert comment.
				- **Evidence Editor:** User can also comment inside the evidence editor itself.
			- **Threads:** Manage comments smoothly with threads.
		- **Set Alert**:
			- User able to set alerts to their email or in-app account. Alerts is important for Canvasm, user can set specific workflows for alert, be it changes in Nodes, Updates in Threads, Data Source Issues.
		- **Changelog:**
	- **Node Organization & Manipulation:** 
		...
		- **Grouping**: Created groups between nodes
...
- Beyond Canvas (`around the core feature`):
	- Homepage:
		- Left Sidebar: 
			- On top right (user profile card)
				- Settings 
			- Personal Canvas:
			- Organization Management:
		- Right Panel:
			- Gallery/List Views of Canvases
			- Shared Resources Management
	- Around Canvas:
		- Application Navigation (Sidebar)
		- Dashboard Page
		- Assets Page (Knowledge Repository)
		- Source Page (Data Governance)
		- Canvas Settings Page
	- Integrated Features:
		- Main/Core Feature:
			- Auto-Dashboard: From Canvas Grouping -> Dashboard
			- Data Governance: Data Source -> Data Governance
			- Blocks Reference: Nodes -> Text Editor 
		- Utilities:
			- Comments
...
HashiCorp Vault - save data warehouse private data
