...
- **Fullkit is Track B made operational.** The approved stack (MotherDuck + Airbyte + Dagster/dbt on the VPS, OpenKnowledge vault, MCP everywhere) is the *analytical* substrate. Fullkit adds the *operational* substrate — the systems of record the warehouse ingests from and the agents act through. Same VPS-first, managed-where-it-matters, flat-cost philosophy; same budget track (B), no new letter.
...
- **Events land in the warehouse from day one.** Order/payment/shipment/message events stream to MotherDuck (via Postgres CDC or outbox → Airbyte). Reporting Tier 2 and the future MMM feature store get their backbone free.
...
```
  CHANNELS                    FULLKIT CORE (VPS + Supabase)                ACTIVATION
  Woo funnels ──┐                                                     ┌─► Retool ops UIs
  Shopee MY/SG ─┤  connectors   ┌──────────────────────────────┐      ├─► Claude via MCP
  Lazada ───────┼──(Airbyte/────┤  S1 Customer & Order Hub     │──────┤   (read + gated write)
  TikTok Shop ──┤   n8n/API)    │  (phone-keyed CDP + orders)  │      ├─► WhatsApp Cloud API
  Fighter ──────┤               │  S3 Inventory (SKU)          │      │   (notify, CS bot)
  WhatsApp ─────┘               │  S4 Money (ledger, commis.)  │      └─► Ads audiences
                                └───────────┬──────────────────┘
  ADAPTERS (commodity)                      │ events (CDC/outbox)
  couriers · gateways · WABA                ▼
                                MotherDuck (S2 + Reporting Tier 2 + MMM panel)
```

**Stack decisions (inherit, don't reopen):**

- **Operational core: Supabase** (Postgres + Auth + Realtime + RLS) — already the proposed data-hub owner in `02 ai operating model`; also literally what Luxana runs on, which de-risks the choice. Self-host on the VPS or managed — decide with the cost model.
...
- **Sync & orchestration:** Airbyte (bulk/scheduled ingestion), n8n (event workflows, notifications), Dagster + dbt (warehouse transforms). All already in the tooling roadmap; Fullkit adds no new orchestrator.
...
- Stand up Supabase core with the **canonical schemas**: `customers` (phone-keyed), `orders`, `order_items`, `payments`, `shipments`, `skus` — the S1/S3/S4 seed. Naming conventions from the vault are the join keys.
- Continuous ingestion: Fighter exports/API + Woo + gateway payouts → Supabase → MotherDuck. One ingestion effort, two landings (operational + analytical). Beat the 7/31-day export windows permanently.
...
- **S1 read model live**: identity resolution on phone across channels — the same stitching the CDP dbt models do, surfaced operationally: customer 360 in Retool for Ida (CS-4 order-update audit becomes a query, not a chase).
...
- Reporting Tier 2 (MER) reads it all from MotherDuck; S2 closes the loop into creative; the CDP's segmentation/attribution now has an operational surface to act through (CRM/retention flows per the dependency map).
...
- **Economics (the thesis):** Fullkit run-cost (VPS + Supabase + WhatsApp per-message + Claude usage) vs displaced SaaS (Fighter tier growth ×5 brands) + labor avoided — tracked in `EFFEN_Cost_Model.xlsx` as its own Track-B line.

## Risks & Open Questions

1. **Single-operator bus factor.** Nadeem builds and runs this *and* the approved Applied AI plan. The mitigation is structural: Fullkit Phase 0–1 is deliberately the *same work* as the scheduled warehouse/CDP builds (one ingestion, two landings), not a parallel program — plus boring managed components (Supabase, Retool), Retool UIs colleagues operate, everything documented in the vault, hand-off bars per the two-bars discipline. Still the #1 risk — scope phases small, and if capacity forces a choice, the approved Gantt wins and Fullkit's write side slips.
2. **Gated boss decisions:** D1 (WhatsApp Business API + per-brand numbers) gates Phase 2 messaging; D3 (WMS budget) gates S3 scope; D7 (PDPA) gates the phone-keyed CDP — consent capture and a legal read are prerequisites, not afterthoughts; D8 (budget envelope) gates everything.
3. **G1 — the CS stack is still unmapped.** Pack 1 (Ida) is unanswered; W2 (ad-lead → close) is uncaptured. The AI closer cannot be specced until it is. Same blocker as the strategic plan — chase Pack 1 first.
4. **Marketplace API access** (Shopee Open Platform, TikTok Shop Partner Center, Lazada Open Platform) requires developer approval with lead times — request in Phase 0, not when needed.
5. **Write-side migration risk.** Order processing is the business's heartbeat; hence read-first strangler, one pilot brand, Fighter kept as fallback through cutover.
6. **Under-building vs Fighter.** Fighter has 6 commission types, 9 couriers, 13 gateways because 470 tenants need the union. We need the intersection EFFEN uses. Resist parity; build from EFFEN's actual workflows (W1–W12), not the incumbents' feature matrix.
7. **Scope creep into SaaS.** The B2B option must not pull multi-tenancy into year 1. Guardrail: no feature ships unless an EFFEN module (CS-x/FN-x/OP-x/MK-x) claims it.
...
4. Add the Fullkit cost lines (Supabase, WhatsApp per-message, marketplace API costs) to `EFFEN_Cost_Model.xlsx` and price the counterfactual (Fighter/Luxana tiers × brands × years).
